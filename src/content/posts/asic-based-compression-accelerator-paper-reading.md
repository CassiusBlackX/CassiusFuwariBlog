...
这篇论文和“基于 DPDK 的压缩设备半虚拟化前后端实现”有较强关联。

我的研究关注的是通过 DPDK compressdev、virtio-compress、QEMU 和 vhost-compress 构建压缩设备半虚拟化前后端。它的核心问题是：如何把压缩能力作为一种虚拟化设备能力暴露给 guest，并在 host 后端高效调度实际压缩资源。

这篇论文则从另一个角度说明：压缩加速器的放置位置对系统效果有决定性影响。

可以对应起来看：

| 论文中的问题 | 与我的方向的关系 |
| --- | --- |
| peripheral / on-chip / in-storage CDPU placement | 我的 virtio-compress 后端可以对接不同 placement 的真实硬件 |
| QAT 8970 / QAT 4xxx | 可作为 vhost-compress backend 的硬件压缩资源 |
| DPZip / DP-CSD | 代表一种 SSD 内嵌压缩路径，说明 host-transparent compression 的优缺点 |
| SR-IOV multi-tenant isolation | 与虚拟化压缩设备的 VF / queue / QoS 设计直接相关 |
| microbenchmark vs application performance | 提醒我的系统评测不能只测 compressdev throughput，还要测 guest 应用性能 |
| data movement dominates latency | 与 rte_comp_op、mbuf、virtqueue、vhost zero-copy 等设计目标一致 |

尤其是论文关于 QAT 多租户隔离的结果，对我的工作很有启发：

如果只是把硬件加速器 passthrough 给不同 VM，或者简单使用 SR-IOV VF，并不一定能获得稳定隔离。虚拟化后端仍然需要做 queue-level scheduling、resource accounting 和 QoS。

对于我的论文，可以借用本文的一个观点：

压缩设备虚拟化不应该只抽象“压缩算法”，还应该抽象和管理“压缩资源”。

也就是说，virtio-compress / vhost-compress 的价值不仅在于让 guest 能用压缩硬件，还在于在多 VM、多队列、多租户场景中提供可控的数据路径、调度策略和隔离语义。
